...
let result = deleteOne("Ciao sono una stringa", true);
console.log(result);
result = deleteOne("Ciao sono una stringa", false);
console.log(result);
...
let rollTheDices = function (n) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    oggetto.summ += risultato;
  }
  return oggetto;
};
let diceLaunch = rollTheDices(4);
console.log(diceLaunch);
...
*/
let isTodayMyBirthday = function (datac, today) {
  let meseOggi = String(today.getMonth());
  let giornoOggi = String(today.getDate());
  let arrayData = datac.split("/");
  let [giornoComplex, meseComplex, annoComplex] = arrayData;
  if (meseOggi === meseComplex && giornoOggi === giornoComplex) {
    return true;
  } else {
    return false;
  }
